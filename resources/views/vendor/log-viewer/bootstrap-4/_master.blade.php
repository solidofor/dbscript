    ...
    <nav class="navbar navbar-expand-md navbar-dark sticky-top bg-dark p-0">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav mr-auto">
                ...
                <li class="nav-item">
                    <a href="{{ route('ping_google') }}" class="nav-link">
                        <i class="fa fa-linux"></i> Ping Google
                    </a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('httpd_status') }}" class="nav-link" onclick="return confirm('Are you sure?');">
                        <i class="fa fa-linux"></i> Apache Status
                    </a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('telegram') }}" class="nav-link">
                        <i class="fa fa-linux"></i> Test Telegram
                    </a>
                </li>
                {{-- <li class="nav-item">
                    <a href="{{ route('tomcat8_status') }}" class="nav-link" onclick="return confirm('Are you sure?');">
                        <i class="fa fa-linux"></i> Tomcat8 Status
                    </a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('tomcat8_restart') }}" class="nav-link" onclick="return confirm('Are you sure?');">
                        <i class="fa fa-linux"></i> Tomcat8 Restart
                    </a>
                </li> --}}
            </ul>
        </div>
    ...
    <script>
        ...
        @if (Session::has('success'))
            @foreach (session('success') as $message)
                @php($success_i++)
                toastr.success("{{ rtrim(print_r($message), '1') }}",'',{timeOut: 500000},'trustedHtml')
            @endforeach
        @endif
        ...
        @if (Session::has('info'))
            @foreach (session('info') as $message)
                @php($info_i++)
                toastr.info("{{ rtrim(print_r($message), '1') }}",'',{timeOut: 500000},'trustedHtml')
            @endforeach
        @endif
        ...
        @if (Session::has('warning'))
            @foreach (session('warning') as $message)
                @php($warning_i++)
                toastr.warning("{{ rtrim(print_r($message), '1') }}",'',{timeOut: 500000},'trustedHtml')
            @endforeach
        @endif
        ...
        @if (Session::has('error'))
            @foreach (session('error') as $message)
                @php($error_i++)
                toastr.error("{{ rtrim(print_r($message), '1') }}",'',{timeOut: 500000},'trustedHtml')
            @endforeach
        @endif
    </script>
</body>
